...
import { productUpdateSchema, type ProductFormData } from '@/lib/validations'
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="p-12 text-center animate-pulse">Loading product data...</div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {(errors as any).name && <p className="text-red-500 text-sm mt-1">{(errors as any).name.message as string}</p>}
            </div>
            <div>
              ...
              {(errors as any).categoryId && <p className="text-red-500 text-sm mt-1">{(errors as any).categoryId.message as string}</p>}
            </div>
              ...
              <div key={field.id} className="p-4 border rounded-lg bg-slate-50 relative group">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    {(errors as any).variants?.[index]?.sku && <p className="text-red-500 text-xs mt-1">{(errors as any).variants[index]?.sku?.message as string}</p>}
                  </div>
                  <div>
                    ...
                    {(errors as any).variants?.[index]?.colorName && <p className="text-red-500 text-xs mt-1">{(errors as any).variants[index]?.colorName?.message as string}</p>}
                  </div>
                  <div>
