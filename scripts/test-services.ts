...
async function testPaymentGatewayService(): Promise<boolean> {
  ...
  try {
    ...
    if (result.success && result.paymentIntent) {
      ...
      logInfo(`Payment Intent ID: ${result.paymentIntent.id}`)
      logInfo(`Amount: LKR ${paymentGatewayService.centsToLkr(result.paymentIntent.amount).toFixed(2)}`)
      logInfo(`Status: ${result.paymentIntent.status}`)
      logInfo(`Client Secret: ${result.paymentIntent.client_secret?.substring(0, 20)}...`)

      // Calculate processing fee
      const fee = paymentGatewayService.calculateProcessingFee(result.paymentIntent.amount)
...
async function testPDFGenerationService(): Promise<boolean> {
  ...
  try {
    ...
    const pdfBuffer = await pdfGenerationService.generateInvoice({
      ...
      paymentInstructions: 'Payment via bank transfer or credit card.',
    })
